...
STRIDE_SECONDS="${STRIDE_SECONDS:-5.0}"
COMPLETION_MARGIN="${COMPLETION_MARGIN:-0.10}"
NEGATIVE_TO_POSITIVE_RATIO="${NEGATIVE_TO_POSITIVE_RATIO:-2}"
KEEP_LAST_WAIT_WINDOWS="${KEEP_LAST_WAIT_WINDOWS:-2}"
FPS="${FPS:-1.0}"
...
MEASURE_OUTPUT_JSON="${MEASURE_OUTPUT_JSON:-${OUTPUT_ROOT}/module_a_seq_lengths_16gb.json}"
MEASURE_OUTPUT_CSV="${MEASURE_OUTPUT_CSV:-${OUTPUT_ROOT}/module_a_seq_lengths_16gb.csv}"
MEASURE_HISTOGRAM_DIR="${MEASURE_HISTOGRAM_DIR:-${OUTPUT_ROOT}/module_a_seq_length_histograms_16gb}"
...
MEASURE_ARGS=(
  python scripts/measure_module_a_seq_lengths.py
  ...
  --completion-margin "${COMPLETION_MARGIN}"
  --negative-to-positive-ratio "${NEGATIVE_TO_POSITIVE_RATIO}"
  --keep-last-wait-windows "${KEEP_LAST_WAIT_WINDOWS}"
  --fps "${FPS}"
