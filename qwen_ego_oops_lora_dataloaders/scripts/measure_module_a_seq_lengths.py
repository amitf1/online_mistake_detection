...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Measure Module A step_id processor token lengths before training.")
    ...
    parser.add_argument(
        ...
        default="step_id",
        help="Prompt/label mode to measure. Default step_id (longer prompts).",
    )
    ...
    for frame_setting, rows_by_split in rows_by_setting_and_split.items():
        for split_name, rows in rows_by_split.items():
            ...
            series = [
                ...
                ("target_tokens", "Target tokens: assistant letter"),
            ]
            ...
            fig.suptitle(f"Module A token lengths: {frame_setting} / {split_name}")
            output_path = output_dir / f"module_a_seq_lengths_{frame_setting}_{split_name}.png"
